...
#ifndef SAUTIL_GET_BITS_H
#define SAUTIL_GET_BITS_H
...
#define SA_INPUT_BUFFER_PADDING_SIZE 64
 ...
 *
 * For examples see get_bits, show_bits, skip_bits.
 */
...
#define UPDATE_CACHE_BE_EXT(name, gb, bits, dst_bits) name ## _cache = \
    SA_RB ## bits((gb)->buffer + (name ## _index >> 3)) << (name ## _index & 7) >> (bits - dst_bits)

#define UPDATE_CACHE_LE_EXT(name, gb, bits, dst_bits) name ## _cache = \
    (uint ## dst_bits ## _t)(SA_RL ## bits((gb)->buffer + (name ## _index >> 3)) >> (name ## _index & 7))
...
#   define SKIP_COUNTER(name, gb, num) \
    name ## _index = SAMIN(name ## _size_plus8, name ## _index + (num))
#endif
...
static inline void skip_bits_long(GetBitContext *s, int n)
{
    ...
    s->index += n;
#else
    s->index += sa_clip(n, -s->index, s->size_in_bits_plus8 - s->index);
#endif
}
 ...
 * Read 0-25 bits.
 */
static sa_always_inline int get_bitsz(GetBitContext *s, int n)
{
...
static inline unsigned int get_bits_long(GetBitContext *s, int n)
{
    ...
    if (!n) {
        return 0;
    } else if ((!HAVE_FAST_64BIT || sa_builtin_constant_p(n <= MIN_CACHE_BITS))
               && n <= MIN_CACHE_BITS) {
        return get_bits(s, n);
    } else {
#if HAVE_FAST_64BIT
 ...
 * @param buffer bitstream buffer, must be SA_INPUT_BUFFER_PADDING_SIZE bytes
                                ...
                                int bit_size)
{
    ...
    if (bit_size >= INT_MAX - SAMAX(7, SA_INPUT_BUFFER_PADDING_SIZE*8) || bit_size < 0 || !buffer) {
        bit_size    = 0;
        buffer      = NULL;
