...
scsi_pt_open_flags(const char * device_name, int flags, int verbose)
{
    ...
    int index, num, got_pd_name, pd_num = 0, share_mode;
...
void
set_scsi_pt_packet_id(struct sg_pt_base * vp ATTR_UNUSED,
                      int pack_id ATTR_UNUSED)
{
}

void
set_scsi_pt_tag(struct sg_pt_base * vp, uint64_t tag ATTR_UNUSED)
{
    struct sg_pt_win32_scsi * psp = vp->implp;
...
set_scsi_pt_task_attr(struct sg_pt_base * vp,
                      int attrib ATTR_UNUSED,
                      int priority ATTR_UNUSED)
{
    struct sg_pt_win32_scsi * psp = vp->implp;
...
int
get_scsi_pt_duration_ms(const struct sg_pt_base * vp ATTR_UNUSED)
{
...
#if 0
int scsi_pt_seek_device(int device_fd, int i_blocks, int verbose)
{
	int index = device_fd - WIN32_FDOFFSET;
	struct sg_pt_handle * shp;
	...
	li_seek.QuadPart = (LONGLONG)i_blocks * 2048;

	li_seek.LowPart = SetFilePointer( shp->fh,
		li_seek.LowPart,
	...
	return li_seek.QuadPart / 2048;
} 

int scsi_pt_read_device(int device_fd, void *p_buffer, int i_blocks, int verbose)
{
	int index = device_fd - WIN32_FDOFFSET;
	struct sg_pt_handle * shp;
	DWORD i_bytes;
	...
	if( !ReadFile( shp->fh, p_buffer,
		i_blocks * 2048,
		&i_bytes, NULL ) )
	{
		i_bytes = GetLastError();
		return -1;
	}
