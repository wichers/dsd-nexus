...
#ifndef SAUTIL_TIMER_H
#define SAUTIL_TIMER_H
...
#if !defined(SA_READ_TIME)
#   if HAVE_GETHRTIME
#       define SA_READ_TIME gethrtime
#   elif HAVE_MACH_ABSOLUTE_TIME
#       define SA_READ_TIME mach_absolute_time
...
#       define SA_READ_TIME ff_read_time
...
#define TIMER_REPORT(id, tdiff)                                           \
    {                                                                     \
        ...
        if (((tcount + tskip_count) & (tcount + tskip_count - 1)) == 0) { \
            ...
            for (i = 0; i < 32; i++)                                      \
                sa_log(NULL, SA_LOG_VERBOSE, " %2d", sa_log2(2*thistogram[i]));\
            sa_log(NULL, SA_LOG_ERROR, "\n");                             \
        }                                                                 \
    }
...
#define START_TIMER                                                         \
    ...
    if (linux_perf_fd == -1) {                                              \
        sa_log(NULL, SA_LOG_ERROR, "perf_event_open failed: %s\n",          \
               sa_err2str(AVERROR(errno)));                                 \
    } else {                                                                \
...
#elif defined(SA_READ_TIME)
#define START_TIMER                             \
    uint64_t tend;                              \
    uint64_t tstart = SA_READ_TIME();           \
